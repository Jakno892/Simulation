 ...
 *  | / ,..´  |    / /_/ / / /_/ /__/ /  / /_/ / / /_/  __/
 *  |/ /______|   /_____/_/\__/\___/_/   \__,_/ /___/\___/
 *
 *
...
#define FLYING_ALTITUDE 1
#define CAM_WIDTH 320
#define CAM_HEIGHT 320

// Function to generate spiral movement
void generate_spiral_movement(double *forward_desired, double *sideways_desired, double *radius, double *angle) {
  *angle += 0.01;
  *radius += 0.01;
  *forward_desired = *radius * cos(*angle);
  *sideways_desired = *radius * sin(*angle);
}
...
int main(int argc, char **argv) {
  ...
  // WbDeviceTag range_height = wb_robot_get_device("range_height");
  // wb_distance_sensor_enable(range_height, timestep);
  ...
  double past_x_global = -2.5;
  double past_y_global = -3;
  ...
  printf("- Use A and D to enable/disable autonomous mode\n");
  ...
  while (wb_robot_step(timestep) != -1) {
    ...
    if (autonomous)
    {
        printf("Generating spiral movement \n");
        // Generate spiral movement
        generate_spiral_movement(&forward_desired, &sideways_desired, &radius, &angle);
        printf("radius: %f \n", radius);
        printf("angle: %f \n", angle);
    }
    ...
    // printf("motor speed M1: %f \n",motor_power.m1);
    // printf("motor speed M2: %f \n",motor_power.m2);
    // printf("motor speed M3: %f \n",motor_power.m3);
    // printf("motor speed M4: %f \n",motor_power.m4);
